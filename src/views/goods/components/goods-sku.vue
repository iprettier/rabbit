    ...
    <dl v-for="item in goods.specs" :key="item.id">
      ...
      <dd>
        <template v-for="val in item.values" :key="val.name">
          <img :class="{selected: val.selected}" @click="changeSku(item,val)" v-if="val.picture" :src="val.picture " alt="" :title="val.name">
...
export default {
  ...
  setup() {
    ...
    // 1. 当你点击的是已选中，只需要取消当前选中
    // 如果点击的是未选中，把 同一个规格的按钮改成未选中，当前点击的改成选中
    /**
     * 选中与取消选中，约定每一个按钮都i拥有自己的选中状态数据
     * @param {Object} val 对象 
     */
